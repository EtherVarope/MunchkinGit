/********************************************************************
	created:	2017/12/29
	created:	29:12:2017   13:29
	filename: 	C:\Users\Administrator\Desktop\UI模型\Test\Test\事件\功能事件\SetACardOfKickTheDoor.cs
	file path:	C:\Users\Administrator\Desktop\UI模型\Test\Test\事件\功能事件
	file base:	SetACardOfKickTheDoor
	file ext:	cs
	author:		DLJ
	
	purpose:	从踢门牌堆中获取一张踢门牌,存放于当前的踢门牌中;
*********************************************************************/
using System;
...
namespace Munchkin
{
    class SetACardOfKickTheDoor : Base_Event
    {
        //Data:
        //--------------------------------------------------------------------------
        ...
        //--------------------------------------------------------------------------
        public SetACardOfKickTheDoor(MomentType momenttype = MomentType.None, int eventID = 6, FunctionObjectType functionobjecttype = FunctionObjectType.None)
        {
        ...
        public override void Execute()
        {
            ...
            //----------------------------------------------------------------
            Card card = null;
            card = UI_PlayerPanel.Instance.ReturnACard();
            Debug.Log(card.GetName());
            if (card!=null)
            {
                
                CoreDataObserver.GetInstance().Set_CardOfKickTheDoorAtPresent(card);

            }
            else
            {
                Console.WriteLine("当前抽取的踢门牌为空");
            }
